...
in a range of astrophysical systems \citep[][]{1977ApJ...218..148M,2016arXiv161001164M}.
...
\mg{I would remove the next paragraph as this seems clear and not necessary to start with an ``apology''.}
...
More involved models with multiple kinematic components can
in principle also be successful.
We focus on presenting two simple and successful models that
...
$\chi^2/d.o.f.>35$. \mg{Why was $35$ chosen as a boundary?}
...
\mg{Sorry but it is not clear to me what this Figure shows \& how it was assembled. I suggest: leave the KS test in the main text as it is straightforward to understand, and move the random trees with more technical details / references about the implementations to the appendix -- and then point to it as a second probe from the main tetx. This way it would break the flow less.}
...
The best models are shown in Figure \ref{multiphaseresults}.
...
Due to the large fraction of \lya photons being emitted within the moving clumps ($P_{\rm cl}$), the `intrinsic' profile represents the kinematics of the clumps, that is, the width of the intrinsic line is set by $\sigma_{\mathrm{cl}}$ and its median offset mainly by $v_{\infty}$ \& $\beta_{\rm cl}$.
Furthermore, the relatively low mean number of clumps per line of sight ($f_{\mathrm{cl}}$) combined with the high velocity dispersion of the clumps implies the 
existence of low-density inter-clump
regions where \lya photons can freely propagate which leads to an emergent spectrum close to the intrinsic one as well as to the high flux at line center as observed.
Because the width of the observable spectrum is hence set primarily by $\sigma_{\rm cl}$, a lower velocity dispersion would produce a narrower line and thus a worse fit.
...
$\sigma_{\rm cl}$, $v_{\infty {\rm cl}}$, $P_{\rm cl}$ which are kept within the derived $1-\sigma$
constraints. \mg{Is $1\sigma$ correct?}
...
However, this view is misleading because merely the best-fit values of (some of) those $3$ parameters, i.e., the corresponding constraint to a given sub-region in the parameter space, make the other parameters less important.
For instance, in our case the large value of $P_{\rm cl}$ makes radiative transfer effects, and therefore other parameters such as the clump column density less relevant for the emergent line profile. However, for instance larger values of $f_{\rm cl}$ -- outside the here considered parameter range -- would re-introduce additional radiative transfer effects and thus increase the number of relevant parameters \citep[see ][for details]{2017arXiv170406278G}. 
...
\mg{I am not sure I understand the rest of this sentence. What do you mean there?}
...
other 3, as the results for the classification tree example in Figure
\ref{fig:tree} shows.
...
\mg{This is interesting. Why do you think that? Could we use that to increase the quoted errors? I am worried that the referee will complain about the small errors.}
...
mass system.  \mg{I am confused -- are much lower rotational velocities not exclude by the priors used?}
...
\bibitem[\protect\citeauthoryear{{Gronke}, {Dijkstra}, {McCourt}  \&
  {Oh}}{{Gronke} et~al.}{2017}]{2017arXiv170406278G}
{Gronke} M.,  {Dijkstra} M.,  {McCourt} M.,   {Oh} S.~P.,  2017, preprint,
  \href {http://esoads.eso.org/abs/2017arXiv170406278G} {} (\mn@eprint {arXiv}
  {1704.06278})
