...
\caption{A representative result from the tree classification algorithm. 
...
number of samples in each of the classification classes.  We use two
classes: low $\chi^2/d.o.f. < 35$ and $\chi^2/d.o.f>35$. 
In this example the best way to increase the probability to find a
model with  low $\chi^2/d.o.f.$ is having $v_{\infty,{\rm cl}}<157.0$ \kms,
...
We tested with a set of 100 additional Monte-Carlo simulations
with random values for $\sigma_{\rm cl}$, $v_{\infty {\rm cl}}$ and
$P_{\rm cl}$ within the derived constraints (and random values for the
other parameters over the whole parameter range) and found that $30$
models result in $\chi^2$ values  
continue to provide a similar $\chi^2$ values as the best $25$ models.  
...
a few parameters \citep{Gronke}, there are the rest of parameters
...
\citep{2012ApJ...751...29Y,LARS,Erb14,Trainor16}. Simple shell models
...
and their rotational velocities are lower than $1$\kms. 
This supports the idea that the multiphase model the correct model
describing \tol, as it provides at least some consistency with
