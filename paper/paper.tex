...
the parameters that can be safeley constrained by \tol's line shape.

From the K-S tests we confirm that only 3 parameters 
...
\jefr{The small uncertainty only says that the increase in
  $\chi^2/dof$ is very steep. I don't think you can get larger errors
  with the available date. By simple inspection of Figure 2 we can
  see that the errors will be small with the data we have. The
  ...
  have there is a huge jump of $\chi^2/dof$ of $\sim 5$ between the minimum
...
moving clumps ($P_{\rm cl}\sim 1$) the `intrinsic' profile represents
the kinematics of the clumps, that is, the width of the intrinsic line
is set by $\sigma_{\mathrm{cl}}$ and its median offset mainly by
$v_{\infty}$ \& $\beta_{\rm cl}$. 
...
of the clumps implies the  existence of low-density inter-clump 
regions where \lya photons can freely propagate which leads to an
emergent spectrum close to the intrinsic one as well as to the high
...
\mg{Is $1\sigma$ correct?}\jefr{What do you mean with
  that question?}
Consistent with the results from the random forest algorithm,
illustrated by the classification tree of Figure \ref{fig:tree},
...
i.e. we cannot randomly pick 11 random values for these
