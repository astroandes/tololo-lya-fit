...
Additionally, if future kinematic maps of tol confirm the velocities
found by either of our models, it would completeley justify the need for
detailed studies to understand the reasons behind a such a high
dynamical mass for a compact dwarf galaxy.
\end{abstract}
...
strong \lya\ emission with two puzzling 
features: the line is highly symmetric, single peaked and wide around the
...
From this evidence we decide to try a multiphase model.
...
For these reasons we also explore a model with a spherical gas
distribution with solid body rotation and without any outflows. 
...
We focus on the existence of two simple and succesful models that
...
We do not have analytic expression for $\hat{f}$. 
For the multi-phase model the results of the MonteCarlo are used to
build histograms that play the role of $\hat{f}$. 
For the rotation model $\hat{f}$ is computed 
has to be computed by numerical integration of analytical
approximations found in  \citet{GaravitoCamargo2014}.
...
A first perspective to put in context \tol's kinematic is comes the
observed scaling relations for elliptic galaxies.
Assuming that \tol\ follows the fundamental plane relationship
between its mean surface brightness $I_e=L_V/2\pi R_{e}^2$, the
projected half-light radius $R_e$ and the velocity dispersion
$\sigma$, described by $\log I_e=1.6 \log\sigma - 1.21\log R_e + 0.55$ \citep{2009ApJ...698.1590G},
...
However, \tol's structural properties, seem to be closer to observed dwarf
elliptical galaxies \citep{2003AJ....126.1794G}. 
Galaxies such as IC 3344 and NGC 4482 have similar absolute $M_V$
magnitudes  and size as \tol. 
These galaxies have velocity dispersions in the range $30$-$45$ \kms. 
The rotational velocities of those systems is lower than $1$\kms. 
This gives support to the idea that the multiphase model might be
closer to be the correct model describing \tol, as it provides at
least kinematic consistency with other dwarf galaxy populations.
