...
$\sigma_{\rm cl}$, $P_{\rm cl}$ which are kept within the derived $1\sigma$
constraints. We find that $30$ models show $\chi^2/d.o.f.<35$.
That is we have a $30\%$ chance to find good models compared with the
lower $1\%$ success rate if the parameters were unconstrained. 
\mg{Is $1\sigma$ correct?}\jefr{I checked and only $\sigma_{cl}$ and
  $P_{cl}$ were run within $1\sigma$. All the other parameters were unconstrained.}

Having constrained only 3 parameters one might wonder why the other 11
parameters do not seem to matter. 
However, this view is misleading because merely the best-fit values of
(some of) those $3$ parameters, i.e., the corresponding constraint to
a given sub-region in the parameter space, make the other parameters
less important. 
For instance, in our case the large value of $P_{\rm cl}$ makes
radiative transfer effects, and therefore other parameters such as the
clump column density less relevant for the emergent line
profile. However, for instance larger values of $f_{\rm cl}$ --
outside the here considered parameter range -- would re-introduce
