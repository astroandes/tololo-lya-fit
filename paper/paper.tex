...
see the review by \cite{2014PASA...31...40D} and the references
therein. 
...
in principle also be sucessful.
...
dynamical mass and argue why the multiphase model should be prefered
...
As we do not have an analytic expression for $\hat{f}$; we use the
...
We do not have a closed analytic expresion for $\hat{f}$, we compute it
...
the clump velocity dipersion, $\sigma_{{\rm cl}}>55.6$ \kms, and the
...
\caption{Enclosed mass in a spherical system as a function of radius.
  Lines correspond to the expectation for dark matter halos and the point
...
In this particular case, we prefer the multiphase model becasue it has kinematic
