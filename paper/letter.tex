...
velocity of the HI gas, with and additional constrain on the typical
size for the emission region, $D$, on could estimate a dynamical mass with 

\begin{equation}
M_{\rm dyn} = \frac{V^{2}D}{G} = 2.16\times10^{5}
\left(\frac{V}{\mathrm{km\ s}^{-1}}\right)^2\left(\frac{D}{\mathrm{kpc}}\right) M_{\odot}
\end{equation}


The multiphase model also sets a typical size for the region where the
clumps are confined.
In the rotation model the size of that region can be infered from the
constraint on the total HI mass.
