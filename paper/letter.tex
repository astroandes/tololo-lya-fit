...
Possible kinematics of an atypical Lyman-$\alpha$ dwarf galaxy}
...
to allow a deep exploration of parameter space and gain, for the firs time,
...
The
$\chi^2$ gap in those $25$ models is close to $\Delta\chi^2 = 3000$, the lowest
...
combination with respect to the observational data. The walkers explore
the parameter space sampling the likelihood function built as $\propto
\exp(\chi^2/2)$, where the $\chi^2$ is computed from each bin in the
...
We estimate the parameter values from the 16th, 50th and 84th
percentiles. 
...
overplot from our best fits from the analytical solution for the multiphase model (thick line). 
and the rotating homogeneous gas sphere (thin line).
