...
\\
\\
JEFR$^{1}$, MG$^3$, MCRG$^1$, JNGC$^2$, MD$^3$
\\
\\
\scriptsize
{$^1$ Bogota
...
translate into a dynamical mass at least $20$ times larger than the
...
multiphase physics under the possible conditions shaping the
...
interpretation so far. Using either bulk rotation or a multiphase ISM,
we manage to explain the \lya\ line in \tol. 
...
The assumed physical size and the velocity dispersion $\sigma_{\rm
  ...
  dyn}=5.9^{+3.1}_{-3.4}\times 10^{9}$ $M_{\odot}$.
...
From the multiphase model we obtained $v=71_{-25}^{+17}$ \kms
and $r=5$ kpc. 
