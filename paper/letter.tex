...
depth $\tau$ measured from the center to the surface, the HI
temperature, $T$, and the velocity at the surface $V_{\rm max}$.  
...
The main effects of rotation are the following.
First, rotation breaks spherical symmetry of the
static case, making the line's observed morphology dependent on the
angle between the line-of-sight and the rotation axis. 

First, the line width increases with rotational velocity.
Second, the intensity at the line's center alto 
In this model rotation increases the width of the line and the flux at
the line center. 
Furthermore, observers at different line of sights
...
medium. The clumps also have a random and an outflowing velocity
component which totals the number of parameters describing the model
to be $14$. In order to map out this large parameter space, we
randomly drew $2500$ sets of parameters within a observationally
realistic range (based on the considerations of
\cite{Laursen2013ApJ...766..124L}) yielding a large variety of
single-, double- and triple-peaked spectra. The full analysis of the
the spectral features as well as more details on the radiative
transfer are presented in \cite{Gronke2016}.   

\textbf{MG: I don't know how much details you want -- should we name
  all of the $14$ (best fit) parameters? Just delete what is
  unnecessary.} 
...
respectively. Furthermore, we found that some parameters such as the
magnitude of the random clump motion $\sigma_{\rm cl}$ improved the
fit significantly whereas others did not. This makes sense also
